...
class Service(BaseService):
    ...
    will check all the services in the order of service_classes to find the correct
    type of service and set it in .service_class so you can start/stop the service
    without worrying about the underlying service type (eg, Systemd, Upstart)
    ...
    @property
    def path(self):
        return self.create_instance().path
    ...
    def create_instance(self):
        ...
        if not self.service_class:
            raise RuntimeError("Could not find a valid service for {}".format(self.name))
    ...
    def _format_cmd(self, action, **kwargs):
        return self.create_instance()._format_cmd(action, **kwargs)

    def is_running(self):
        return self.create_instance().is_running()
